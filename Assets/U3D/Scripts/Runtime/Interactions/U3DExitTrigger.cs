...
{
    /// <summary>
    /// Enhanced trigger component for player exit events
    ...
    public class U3DExitTrigger : NetworkBehaviour
    {
        ...
        [Header("Events")]
        [Tooltip("Called when trigger is exited")]
        public UnityEvent OnExitTrigger;

        [Tooltip("Called when U3D player specifically exits")]
        public UnityEvent OnPlayerExit;

        [Tooltip("Called when any valid object exits")]
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            if (detectU3DPlayer)
            {
                ...
                if (playerController != null)
                {
                    shouldTrigger = true;
                    isPlayer = true;
                    Debug.Log($"U3D Player exited trigger: {gameObject.name}");
                }
            }
            ...
            if (!shouldTrigger && detectTaggedObjects && !string.IsNullOrEmpty(requiredTag))
            {
                if (other.CompareTag(requiredTag))
                {
                    ...
                    Debug.Log($"Tagged object ({requiredTag}) exited trigger: {gameObject.name}");
                }
            }
        ...
        private void ExecuteTrigger(bool isPlayer, GameObject triggerObject)
        {
            ...
            Debug.Log($"Exit trigger executed on {gameObject.name} by {triggerObject.name}");
        }
        ...
        public void ResetTrigger()
        {
            ...
            Debug.Log($"Exit trigger reset: {gameObject.name}");
        }
        ...
        private void OnValidate()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                {
                    Debug.LogWarning($"U3DExitTrigger on {gameObject.name} requires a trigger collider");
                }
            }
        }
    }
